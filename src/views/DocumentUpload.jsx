...
const DocumentUpload = (props) => {
  ...
  const call_upload_document = async (data) => {
    ...
    RNFetchBlob.fetch('POST', api_url + image_upload, {
      ...
      setLoading(false);
      dropDownAlertRef.alertWithType('error', 'Validation error', 'Error on while upload try again later');
    })
  }
    ...
    await axios({
      ...
      .then(async response => {
        ...
        if (response.data.status == 1) {
          ...
          setStatus(15);
          dropDownAlertRef.alertWithType('success', 'Success', 'Successfully uploaded your document');
        }
      })
      .catch(error => {
        setLoading(false);
